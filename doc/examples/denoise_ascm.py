...
def ascm_comparision(S0, A1, A2, B1, B2):
    axial = S0.shape[2] / 2
    sigma = estimate_sigma(S0, N=4)
    # this is reported to have the top performer
    filterd_ornlm = np.array(ascm.ascm(S0, A1, A2, sigma[0]))
    # this is reported to have the top performer
    filterd_nlmeans = np.array(ascm.ascm(S0, B1, B2, sigma[0]))
...
if __name__ == '__main__':
    ...
    # h=0.01*mv
    # We estimate the variance of the noise present using estimate_sigma
    # function
